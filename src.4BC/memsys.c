...
uns64 memsys_access_modeBC(Memsys *sys, Addr lineaddr, Access_Type type,uns core_id){
  ...
  if(type == ACCESS_TYPE_STORE){
    ...
    if(needs_dcache_access){
        Flag outcome=cache_access(sys->dcache, lineaddr, is_write,core_id); // dcache access
        
        //dcache miss
        if(outcome==MISS){
            cache_install(sys->dcache, lineaddr, is_write,core_id); // dcache miss
            ...
            if((sys->dcache->last_evicted_line.dirty == TRUE) && (sys->dcache->last_evicted_line.valid == TRUE)){
                ...
                set_index = lineaddr & (sys->dcache->num_sets-1);
                evict_line = sys->dcache->last_evicted_line.tag * sys->dcache->num_sets + set_index;
        ...
        Flag outcome=cache_access(sys->icache, lineaddr, is_write,core_id); // icache access
        
        if(outcome==MISS){
            cache_install(sys->icache, lineaddr, is_write,core_id); // icache miss

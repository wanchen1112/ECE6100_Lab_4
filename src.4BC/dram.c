...
#define DRAM_T_ACT         45   // t_col
#define DRAM_T_CAS         45   // t_row
#define DRAM_T_PRE         45   // t_pre
#define DRAM_T_BUS         10   // t_ctrl
...
uns64   dram_access_extra_credit(DRAM *dram,Addr lineaddr, Flag is_dram_write){
  uns64 delay=0;
    ...
    uns bank_id, line_in_row;
    Addr row_in_bank;
    
    delay += DRAM_T_BUS+DRAM_T_CAS;
    
    line_in_row = lineaddr & (ROWBUF_SIZE/CACHE_LINESIZE -1);
    bank_id = (lineaddr / (ROWBUF_SIZE/CACHE_LINESIZE)) & (DRAM_BANKS-1);
    row_in_bank = (lineaddr) / (ROWBUF_SIZE/CACHE_LINESIZE) / (DRAM_BANKS);
    
    //printf("bank_id: 0x%x, row_in_bank: 0x%x\n", bank_id, row_in_bank);
    
    // Assume a mapping with consecutive lines in the same row
    
    if(dram->perbank_row_buf[bank_id].valid == TRUE){
    
        if(dram->perbank_row_buf[bank_id].rowid == row_in_bank){ // RB hit
